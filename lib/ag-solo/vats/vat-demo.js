...
function build(E, D, log) {
  ...
  async function startup(http, pubsub) {
    ...
    const publish = str => E(pubsub).publish('canvas', str);
    E(pubsub).subscribe('canvas', harden({ next: stateChangeHandler }));

    const canvasSize = 10;
    const gallery = makeGallery(E, log, publish, canvasSize);
    publish(gallery.readFacet.getState());
