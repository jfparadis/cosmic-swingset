    ...
    return {
      async processInbound(obj) {
        ...
        if (type === 'autoswapGetPrice') {
          const { contractId, extent0, desc0, desc1 } = data;
          const exchange = await userFacet.getPrice(contractId, extent0, desc0, desc1);
          return { type: 'autoswapPrice', data: exchange.extent };
        }

        if (type === 'autoswapGetOfferRules') {
          const { contractId, extent0, desc0, desc1 } = data;
          const offerRules = await userFacet.getOfferRules(contractId, extent0, desc0, desc1);
