...
// Avoid creating empty strings before checking condition.
...
export function makeWallet(
  ...
  const pursesState = {}; // Serializable object
  ...
  async function updatePursesState(purseName, purse) {
    ...
    const { label: { description }, extent } = balance;
    pursesState[purseName] = { description, extent };
  ...
  function makeEmptyPurse(purseName, assay) {
    // TODO sanitize name
    nameToPurse.has(purseName) &&
      throwTypeError(`Purse name already used in wallet.`);
  ...
  function getAssays() {
    return harden(Array.from(allAssays));
  }

  async function depositPayment(purseName, payment) {
    nameToPurse.has(purseName) || throwTypeError(`Purse not found in wallet.`);
