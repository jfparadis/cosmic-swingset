...
export async function makeExchange(E, log, host, zoe, registrar, uploads) {
  ...
  async function getInstanceId(contractId, assay0, assay1) {
    ...
    // 3. Store 2-way for simpler lookup.
  ...
  ASSAY_SETUP.forEach(async ([desc, extent]) => {
    ...
    const assayRid = await E(registrar).register(desc, assay);
    console.log('Registrar id', assayRid, 'for assay', desc);
  ...
  async function getPrice(contractId, extent, assayRid0, assayRid1) {
    const assayR0 = await E(registrar).get(assayRid0);
    const assayR1 = await E(registrar).get(assayRid1);
    const instanceId = await getInstanceId(contractId, assayR0, assayR1);
    ...
    const isNormal = checkOrdering(assay0, assay1, assayR0, assayR1);
  ...
  async function getOfferRules(contractId, extent, assayRid0, assayRid1) {
    const assayR0 = await E(registrar).get(assayRid0);
    const assayR1 = await E(registrar).get(assayRid1);
    const instanceId = await getInstanceId(contractId, assayR0, assayR1);
    ...
    const offerDesc0 = {
      rule: 'offerExactly',
      assetDesc: { assay: assayRid0, extent },
    };
    const offerDesc1 = {
      rule: 'wantAtLeast',
      assetDesc: { assay: assayRid1 },
    };

    const offerRules = harden({
      offerDesc: [
        isNormal ? offerDesc0 : offerDesc1,
        isNormal ? offerDesc1 : offerDesc0,
        {
          rule: 'wantAtLeast',
          assetDesc: {},
        },
    ...
    const instanceRid = await E(registrar).register('contractId', instanceId);

    return { offerRules, instanceRid };
  }
