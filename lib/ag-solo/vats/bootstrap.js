      ...
      function makeBundler(contractHost, vats) {
        return harden({
          async createDemoBundle(nickname) {
            ...
            const walletBundle = await E(vats.wallet).createBundle();
            const exchangeBundle = await E(vats.exchange).createBundle();
      ...
      return harden({
        async bootstrap(argv, vats, devices) {
          ...
          if (ROLES.three_client) {
            ...
            await E(vats.pixel).startup(host);
            const zoe = await E(vats.zoe).getZoe();
            const uploads = await E(vats.uploads).getUploads();
            const registrar = await E(vats.registrar).getSharedRegistrar();
            await E(vats.exchange).startup(host, zoe, registrar, uploads);
            await E(vats.wallet).startup(host, zoe, registrar);
            ...
            await E(vats.http).setCommandDevice(devices.command, {client: true});
            await E(vats.wallet).setCommandDevice(devices.command, {client: true});
            D(devices.command).registerInboundHandler(vats.http);
            await E(vats.http).setPresences(bundle, await createPrivateBundle(vats));
