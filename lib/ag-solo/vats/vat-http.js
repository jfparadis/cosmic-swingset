...
function build(E, D) {
  ...
  return {
    ...
    async setPresences(ps) {
      Object.assign(homeObjects, ps);
    },

    nextUserState(key, state) {
      if (key !== 'canvas') {
        console.log(`not listening for ${key} state`);
        return;
      }

      console.log('Updating canvas to', state);
      if (!state) {
        return;
      }
      // console.log(`updating canvas state ${state}`);
      canvasState = state;
      if (!commandDevice) {
        return;
      }
