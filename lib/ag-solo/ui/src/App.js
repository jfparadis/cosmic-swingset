...
function Repl({ history, connected, sendCommand }) {
  ...
  const { stayScrolled/*, scrollBottom*/ } = useStayScrolled(historyRef);
  useLayoutEffect(() => {
    stayScrolled();
  }, [history]);

  const onChange = (event) => { 
    setCommandEntry(event.target.value);
  };
  ...
  history</div>
      <div id="history" ref={historyRef} className="history">{historyElements}</div>
      <div className="history" >
        <div id="command-entry" className="command-line">
          ...
          <div><input id="input" tabIndex="0" type="text" style={{backgroundColor: bgColor}}
            value={commandEntry} onChange={onChange} /></div>
...
function App() {
  ...
  return (
    <div>
      <Connector
        wsURL={socketEndpoint}
        messageHandler={messageHandler}
        updateConnected={setIsOnline}
      />
