...
function Repl({ history, connected, sendCommand }) {
  ...
  const [historyI, setHistoryI] = useState(null);
  ...
  function moveHistory(delta) {
    ...
    console.log(`history from ${historyI} by ${delta} for ${history.length}`);
    let newI = historyI;
    if (historyI === null) {
      // null means we are working on a new entry rather than from a copied history entry
      setEntryStore(commandEntry);
      newI = history.length;
    }
    newI += delta;
